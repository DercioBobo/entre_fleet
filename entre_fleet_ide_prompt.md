...
|---|---|---|---|
| `Fleet Vehicle` | `FleetVehicle` | `field:license_plate` | Matricula, marca, modelo, ano, categoria, estado, tipo de combustivel, odometro atual, chassi, motor, condutor atribuido (Link -> Fleet Driver, **read-only, synced from the active Fleet Driver Assignment — see validation logic**), validade seguro/inspecao/licenca (the 3 core documents every vehicle has — see Document Tracker note below), numero de eixos (default 3) + numero de baterias (default 1) driving auto-synced `tyres`/`batteries` child tables (see below), foto, observacoes |
...
| `Fleet Vehicle Tyre` | `FleetVehicleTyre` | — (child table, `istable: 1`, no autoname) | Posicao (Data, read-only, e.g. "2º Eixo Direito"), estado (Novo/Recauchutado/Usado/A Substituir), ultima alteracao — rows of `Fleet Vehicle`'s "tyres", auto-generated/synced from `axle_count` in `Fleet Vehicle.validate()` (preserves existing row state when the count changes) |
| `Fleet Vehicle Battery` | `FleetVehicleBattery` | — (child table, `istable: 1`, no autoname) | Posicao (Data, read-only, e.g. "Bateria 2"), estado (Nova/Usada/A Substituir), ultima alteracao — rows of `Fleet Vehicle`'s "batteries", same auto-sync pattern from `battery_count` |
...
| `Fleet Maintenance Schedule` | `FleetMaintenanceSchedule` | `naming_series:` | Link -> Fleet Vehicle, Link -> Fleet Maintenance Type, intervalo (dias, pre-filled from the type's default when empty), ultima realizacao, proxima data agendada (calculado), estado (Agendado/A Vencer/Vencido) — one record per recurring maintenance item per vehicle; ships a native Frappe Calendar View (`fleet_maintenance_schedule_calendar.js`) keyed on proxima data agendada, plus a form button that creates a pre-filled Preventiva `Fleet Maintenance Request` when due |
| `Fleet Maintenance Type` | `FleetMaintenanceType` | `field:type_name` | Nome, activo (checkbox), intervalo padrao (dias) — standardized master list of recurring maintenance items (Mudanca de Oleo, Rotacao de Pneus, etc.) so `Fleet Maintenance Schedule` isn't free text prone to drift across vehicles |
...
- **Workspace ("Entre Fleet", `entre_fleet/entre_fleet/workspace/entre_fleet/entre_fleet.json`)**: shortcuts (Visão Geral dashboard, Ficha do Veículo page, Calendário de Manutenções, Veículos, Pedidos de Manutenção) + link cards to every doctype grouped into 4 cards (Veículos e Condutores / Operação / Manutenção / Documentos). No Number Cards embedded directly — verified against Frappe v14.19.1 source (`workspace.js`'s EditorJS `tools` registration) that the Workspace content-block editor in this version only supports `header`/`paragraph`/`chart`/`card`/`shortcut`/`quick_list`/`spacer` blocks, not `number_card`.
- **Dashboard ("Entre Fleet", `/app/dashboard-view/Entre Fleet`, reachable via the workspace's "Visão Geral" shortcut)**: the fleet-wide "what needs attention" picture, using the doctype actually built for Number Cards/Charts. 7 `Number Card` fixtures (`entre_fleet/entre_fleet/number_card/*/`) — Veículos Activos, Veículos em Manutenção, Documentos a Expirar/Expirados (Fleet Document Tracker only — the Vehicle's own 3 built-in expiry fields aren't backed by a stored status column, so they're not Number-Card-filterable the same way; the Dossier's chips cover those per-vehicle), Manutenção Preventiva a Vencer/Vencida, Pedidos de Manutenção Abertos, Custo de Combustível (Este Mês), Custo de Manutenção (Este Mês). Plus 2 `Dashboard Chart` fixtures (`entre_fleet/entre_fleet/dashboard_chart/*/`): a Sum/timeseries line chart (fuel cost trend) and a Group By/Sum bar chart (maintenance cost per vehicle — this is why `Fleet Job Card` now has a `vehicle` field, `fetch_from: maintenance_request.vehicle`, since Group By needs a real field on the charted doctype, not one two hops away). `filters_json`/`group_by_*` fields verified against a real shipped ERPNext example (`erpnext/accounts/number_card/total_incoming_bills`) and the `dashboard_chart.py` controller's `check_required_field()`, not guessed.
